...
context('Ejecution Errors', () => {
  ...
  it('Runing a program with sintax errors', () => {
    const program = 'DIV R1 0x000'
    cy.get('[id="ace-editor"]')
      .type(getProgram([program]))

    cy.get('[id="execute-button-id"]').click()

    cy.get('[id="client-snackbar"]').should('have.text', 'Hubo un error, es posible que sea de sintaxis')
    cy.get('[id="results-box-id"]').should('have.value', `Tu programa contiene al menos una instrucción incompleta o erronea: \n\nInstrucción fallida: ${program}\nEn la linea: 1`)
    cy.get('[class*="ace_error"]').click()
    cy.get('[class="ace_tooltip"]').should('have.text', 'Error de sintaxis')
  })
