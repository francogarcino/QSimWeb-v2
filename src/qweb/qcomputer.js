...
class Computer {
  ...
  __get_registers() {
    return [...Array(parseInt(qConfig.getItem('registers_number'))).keys()].map(n => new Register(n))
  }
  ...
  _execute_cycle(cell) {
    ...
    const assembled_instruction = instruction.disassemble(instruction, cell_value, this.state)
...
class State {
  ...
  load(binary) {
    binary.split(/(.{16})/).filter(O => O).forEach(cell => {
  ...
  update_ir(value, descriptive_value) {
    var get_ir = (ir, val) => { return this.new_instruction ? String(val) : `${ir}${String(val)}` }
  ...
  read_memory(cell_id) {
    ...
    if (value === undefined && this.current_stage === STAGE_FETCH && this.organic_pc) {
      throw new (this.empty_stack() ? ExcecutionFinished : IncompleteRoutineError)()
    }
    else {
      this.MBR = hex2dec(value !== undefined ? value : this.get_default_value(cell_id))
    }
  ...
  get_default_value(address) {
    ...
    const default_value = new DefaultCellValue().find_subclass(default_value_name)
  ...
  write_memory(cell_id, value) {
    this.save_action(this.loading ? ACTIONS.ASSEMBLE : ACTIONS.WRITE_MEMORY, { cell: cell_id, value })
